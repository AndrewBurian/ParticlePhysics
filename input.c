...
void
handleInput (struct simulation *sim, struct universe *univ,
	...
	case SDL_MOUSEBUTTONDOWN:
	  ...
	  if (event.button.button == SDL_BUTTON_LEFT)
	      ...
	      if (sim->state == SIMULATION_NORMAL)
		{
		  ...
		  double x =
		    ((event.button.x -
		      (render->width / 2)) / render->scale) - render->xPos;
		  double y =
		    ((event.button.y -
		      (render->height / 2)) / render->scale) - render->yPos;
	      ...
	      else if (sim->state == SIMULATION_UPDATEPARTICLE)
		{
		  if (sim->hotParticleState == HOTPARTICLE_VELOCITY)
		    {
		      sim->state = SIMULATION_UPDATEPARTICLE_VELOCITY;
		    }
		}
	    }
	      ...
	      if (sim->state & SIMULATION_UPDATEPARTICLE)
		{
		  ...
		  if (sim->hotParticleState >= HOTPARTICLE_COUNT)
		    {
	  ...
	  if (event.button.button == SDL_BUTTON_LEFT)
	    {
	      ...
	      if (sim->state == SIMULATION_UPDATEPARTICLE_VELOCITY)
		{
		  sim->state = SIMULATION_UPDATEPARTICLE;
		}
	      else if (sim->state != SIMULATION_UPDATEPARTICLE)
		  ...
		  if (sim->last_click_x == event.button.x
			...
			((event.button.y -
			 ...
			 render->scale) - render->yPos;
		      ...
		      sim->hotParticle = addParticle (univ, &p);
		      sim->hotParticleState = HOTPARTICLE_VELOCITY;
	  ...
	  if (sim->state == SIMULATION_PAN)
	    {
	      ...
	      render->xPos += event.motion.xrel / render->scale;
	      render->yPos += event.motion.yrel / render->scale;

	    }
	  else if (sim->state == SIMULATION_UPDATEPARTICLE_VELOCITY)
	    {

	      struct particle *p = &univ->particles[sim->hotParticle];
	...
	case SDL_MOUSEWHEEL:
	  ...
	  if (sim->state == SIMULATION_UPDATEPARTICLE
	      ...
	      if (sim->hotParticleState == HOTPARTICLE_MASS)
		{
	      ...
	      else if (sim->hotParticleState == HOTPARTICLE_CHARGE)
		{
		  struct particle *p = &univ->particles[sim->hotParticle];
