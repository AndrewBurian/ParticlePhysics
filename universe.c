...
int
addParticle (struct universe *univ, struct particle *p)
{
...
void
deleteParticle (struct universe *univ, struct particle *p)
{
...
int
getParticle (struct universe *univ, double xPos, double yPos)
{
  int i;
  struct particle *p;
  ...
  for (i = 0; i < univ->nextParticle; i++)
    {
      p = &univ->particles[i];
      if (sqrt (pow (xPos - p->xPos, 2) + pow (yPos - p->yPos, 2)) <= p->size)
	{
	  return i;
	}
    }
...
void
universeExpand (struct universe *univ)
{
  ...
  univ->particles =
    realloc (univ->particles, univ->particleCount * sizeof (struct particle));
...
int
readFileLine (char **line, size_t * n, FILE * file)
{
  int count = 0;
  int i = 0;
...
struct universe *
universeInitFromFile (FILE * file)
{
  struct universe *univ = malloc (sizeof (struct universe));
  char *line = 0;
  size_t lineSize = 0;
...
void
saveToFile (struct universe *univ)
{
  char fileName[20] = { 0 };
  FILE *file = 0;
  int fileCount, i;
...
void
freeUniverse (struct universe *univ)
{
